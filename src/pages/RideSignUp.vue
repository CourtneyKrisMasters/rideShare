<!-- allows a passeger to sign up for a ride -->
<template>
  <v-container>
    <div>
...
//gets all rides for the table
 mounted: function() {
    ...
    this.$axios.get("/rides").then((response) => {
      this.rideItems = response.data.details.map((ride) => ({
        id: ride.id,
...
methods: {
    ...
    rideSignUp(rideId) {
      this.$axios.post(`/passengers/${this.$store.state.currentAccount.id}/rides/${rideId}`)
       .then((result) => {
